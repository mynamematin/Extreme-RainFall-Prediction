...
lead_time = 0
model_path = "/home/erpas_rnd/Abhijit/convlstm_model_temp/7in_1out/7in_1out.h5"
save_path = "/home/erpas_rnd/Abhijit/convlstm_model_temp/7in_1out/"

def preprocess_data(sequence, n_steps,channels):
       
    #separating nan and non nan points and doing exp space transform
    sequence = sequence.sel(time=slice("1998-01-01","2019-12-31"))
...
def create_input(channels, n_steps):
    ...
    for i in range(len(channels)):
        ...
        input, _ = preprocess_data(xr.open_dataarray(f"/home/erpas_rnd/Abhijit/data/data/{channels[i]}.nc"), n_steps=steps,channels=channels[i])
        input = np.expand_dims(input,axis=2)
        print(f"Adding channel {i} with shape: {input.shape}")
        stack.append(input)
    out = np.dstack(stack)
    return out
channels=["file_name","file_name"]
steps=7
input = create_input(channels, steps)
print("########################put the path of rainfall data#########################################################")
_, target = preprocess_data(xr.open_dataarray("/home/erpas_rnd/Abhijit/data/data/t2m_10_20.nc"), n_steps=steps,channels=channels[-1])
...
seq.add(ConvLSTM2D(filters=4,kernel_size=(3,3),padding='same',input_shape=(6,137,201,2),return_sequences=True,data_format='channels_last'))
